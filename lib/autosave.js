...
module.exports = {
  ...
  activate() {
    ...
    this.subscriptions.add(lumine.window.onWillDestroy(() => this.subscriptions.dispose()));
  ...
  autosavePaneItem(paneItem, create = false) {
    ...
    // them automatically.
    if (lumine.config.get("core.promptOnSaveConflictedFile") && paneItem.isInConflict?.()) {
      return;
    }
